...
def static_payload(
    ...
    if source_image_path is not None:
        encoded = base64.b64encode(source_image_path.read_bytes()).decode("ascii")
        payload["init_images"] = [{"type": "base64", "base64": encoded}]
...
def generate_static(
    ...
    **_: Any,
    ...
    if "init_images" in payload_for_log:
        payload_for_log["init_images"] = [{"type": "base64", "base64": "[base64 omitted]"}]
    append_trace(
        trace_path,
        "provider_request",
    ...
    if status >= 400:
        raise RuntimeError(f"PixelLab static generation failed with HTTP {status}.")
